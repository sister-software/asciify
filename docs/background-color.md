# Per-cell background color in `AsciifyTerminal`
...
water, an empty status region). This adds a fourth per-cell channel: background color.
...
**The sentinel.** A packed canonical color occupies bits 0–23 (`truecolor` packs 8-bit RGB into 24 bits;
`ansi256` never exceeds index 255), so `0` is a legitimate color — black — and can't double as "no
background set": a cell explicitly painted black would be indistinguishable from a cell nobody ever
touched. `NO_BACKGROUND = 0x01_00_00_00` sits one bit above the packed range, so no canonical color can
...
existing `color` parameter — the old four-argument call shape still works unchanged. It's canonicalized
through the same `_canonicalColor()` path as the foreground, so `ansi256` quantization applies to both.
...
`_canonicalColor` path so `ansi256` quantization applies. Its parameter type matches `fillBackground`'s —
...
otherwise canonicalize) is factored into one shared `_canonicalBackground()` helper that both
...
how it already resets chars to `SPACE` and colors to `0`.
...
1. **Damage detection now compares backgrounds too** — both the seek `while` loop and the span-extension
   `for` loop check `backgrounds[i] !== previousBackgrounds[i]` alongside chars and colors. Without this,
...
3. **`activeBackground`** tracks the background SGR state the way `activeColor` tracks foreground, but
   initializes to `NO_BACKGROUND` rather than `-1`. Every payload already ends in `SGR_RESET`, so the
...
- **Byte-identical regression** (the one that matters most): reruns the existing "addresses only the
  damaged span on a single-cell change" scene at the _default_ (truecolor) color depth instead of `none`
...
- Setting a background doesn't disturb an existing character or foreground: paint a cell with `setCell`,
  ...
  color from the earlier `setCell` call are still in the output untouched.
...
comparisons in `_emitDamage`** (reverting just those two comparisons to chars+colors only, leaving
...
- **`setSize()` (and therefore any resize) wipes backgrounds back to the sentinel**, the same way it
  already wipes chars/colors. A resize-driven repaint means backgrounds must be re-applied afterward, same
  as before for foreground colors.
