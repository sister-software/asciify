...
`tui/` is a separate entry point (`@sister.software/asciify/tui`) that rasterizes to ANSI escapes instead of a canvas. **`AsciifyTerminal` is a sibling of the canvas renderers, not a fourth `AsciifyBase` subclass** — the base's sizing contract is pixel-space (`setSize` in CSS px × `pixelRatio`, `getBoundingClientRect`, scratch 2D context), none of which means anything in character space. The API matches where the meaning matches (`columnCount`/`rowCount`, `setOptions`, `applySizeTo`, `rasterize(buffer, flipY)`); `setSize` honestly takes columns and rows, defaulting to the output's own `columns`/`rows`.

Things to know before editing:

- **Nothing under `tui/` may import `utils/canvas.ts` or anything DOM-typed.** The subpath ships DOM-free declaration files so Node consumers without a `dom` lib can use it. It declares its own structural types instead — `TerminalLike` is the same move as `WebGLRendererLike`, aimed at `process.stdout`. `LuminanceCharacterMap` and the `options/common.ts` constants are safe imports; that's the shared vocabulary with the canvas side.
- Two modes, differing only in the source-size multiplier `applySizeTo` requests: `glyph` (one source pixel per cell, through the shared `LuminanceCharacterMap`) and `braille` (2×4 per cell, Bayer-dithered into the Unicode braille block, whose codepoints are `0x2800 + dot bitmask` — the bit layout lives in `tui/braille.ts`, public because a revamped mapscii will consume it, and descends from mapscii/node-drawille). The 2×4 subcell is roughly square, which is what sidesteps the terminal cell-aspect problem.
- **The frame-to-frame damage diff lives here on purpose.** The canvas renderers dropped theirs because rebuilding wholesale beat per-cell bookkeeping; a terminal's budget is bytes down a possibly-remote wire, where the diff wins by orders of magnitude. An unchanged frame writes zero bytes — keep it that way. Colors are canonicalized _per depth_ before diffing (at `ansi256`, two RGB values quantizing to the same palette index must not count as damage), color escapes are emitted only on change, and inkless cells (spaces, dotless braille) never touch color state.
- Every row is cursor-addressed relative to `options.origin`, which is what makes the renderer safe inside a pane it doesn't own (an Ink box, a split). It never writes outside `origin + columnCount × rowCount`. If something else draws over the pane, `invalidate()` forces a full repaint.
- The cell-level API is public on purpose: `setCell()` places characters directly (labels, cursors — mapscii's `setChar` path) and `flush()` diffs-and-writes without rasterizing. Geometry-producing consumers use these instead of RGBA buffers; the ANSI helpers (`cursorTo`, `quantizeToAnsi256`, sync brackets) and the braille dot table are exported for the same reason.
- Frames are bracketed in DEC private mode 2026 (synchronized output) by default — terminal vsync, harmlessly ignored where unsupported.

Three Node demos, distinct stress patterns, all needing `yarn compile` first: `demo/tui/spiral.mjs` runs the spiral demo's fragment shader ported to CPU JavaScript — a reminder that a shader is just a function of `(u, v, time)` samplable at any grid — and every cell changes every frame, so it costs what a full repaint costs. `demo/tui/ball.mjs` (Amiga Boing homage) is the damage diff's showcase: static background, moving ball, ~15% of a full repaint's bytes per frame. `demo/tui/3d.mjs` is a CPU raytrace of `demo/3d`'s Three.js scene — same camera, animation, palette, and facets (flat shading approximated by quantizing the normal to the geometry's segments) — with OrbitControls traded to arrow keys/WASD and `+`/`-`. `demo/tui/3d-gpu.mjs` is the same scene through a **real GPU pipeline**: the actual Three.js scene graph runs in headless Chromium (Playwright, already a devDependency for the tests), the page does `readPixels` in the same task as the draw, and POSTs each frame to the Node process, whose response carries camera + pane size back — one request/response loop is the frame clock, control channel, and resize protocol. Every demo takes `--size=<cols>x<rows>` (runs without a TTY, frames pipe anywhere) and `--time=<ms>` (pins the animation clock), parsed by `parseDemoArguments` in `demo/tui/common.mjs`. Together they make frames deterministic — that's how the CPU and GPU 3D demos are held to agreement: capture both at the same pinned time and size, parse the escapes back into cell grids, and diff by region. The CPU port's fidelity subtleties all live in `3d.mjs` comments: shading happens in linear space, the grid blends in gamma space (GL blends against the encoded framebuffer), and the sphere's albedo is linear because `Color.setHSL` interprets arguments in Three's working color space while CSS-string colors are sRGB. They share `demo/tui/common.mjs` for session plumbing; the part worth knowing is that **Ctrl-C cannot be trusted to arrive as SIGINT** — hosts that hold the terminal in raw mode (tmux, IDE terminals, agent harnesses) deliver it as a `0x03` byte on stdin, so the helper reads stdin raw and treats `0x03`/`q` as quit, keeping signal handlers only as fallback. Tests are in `test/tui.test.ts`; they're pure string assertions and need no browser APIs, but run under the same Vitest browser setup as everything else.
